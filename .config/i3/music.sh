...
play_youtube_audio() {
    ...
    mpv --no-video --script=/etc/mpv/scripts "$video_url" &
...
action=$(echo -e "󰐊 Play Music\n󰓛 Stop Music" | rofi -dmenu -i -p "Select Action:"  -theme-str '@import "/home/sairu/.config/rofi/Music.rasi"')
...
if [ "$action" == "󰐊 Play Music" ]; then
    ...
    query=$(rofi -dmenu -p "Search YouTube:"  -theme-str '@import "/home/sairu/.config/rofi/Music.rasi"')
    ...
    selection=$(echo "$results" | rofi -dmenu -i -p "Select Video:"  -theme-str '@import "/home/sairu/.config/rofi/Music.rasi"')
