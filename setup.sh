...
declare -a aur_packages=(
    ...
    nm-connection-editor noto-fonts-emoji tldr stow wl-clipboard unzip yt-dlp kitty exa zsh grim
    slurp swaylock-effects
)

declare -a font-packages=(
    ttf-roboto-mono-nerd ttf-jetbrains-mono-nerd ttf-noto-nerd ttf-hack-nerd ttf-firacode-nerd ttf-segoe-ui-variable
)

install_arch() {
    sudo pacman -S "${common_packages[@]}" github-cli fd git-delta lazygit ttf-firacode-nerd wl-clipboard  --needed --noconfirm
}
...
install_packages() {
    ...
    endeavouros) color="033" && install_arch ;;
...
install_oh_my_zsh() {
    ...
    git clone "$gh/z-shell/F-Sy-H"
    git clone "$gh/chrissicool/zsh-256color"
    git clone "$gh/djui/alias-tips.git"
    git clone "$gh/Aloxaf/fzf-tab"
    git clone "$gh/unixorn/git-extra-commands.git"
    git clone "$gh/hlissner/zsh-autopair"
...
declare -a home_files=(
    "zshrc/.zshenv" ".bashrc" ".dircolors" ".dmenurc" ".gitconfig" ".inputrc" ".luarc.json"
